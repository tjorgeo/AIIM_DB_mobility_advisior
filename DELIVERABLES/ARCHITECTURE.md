...
| `user_onboardings` | Preferences & context | `score_emission`, `score_money`, `score_flexibility` (0-100), `preferred_transport_modes[]`, `mobility_budget_monthly_eur`, employment/household fields, `travel_statement`, `activity_statement`. |
...
| `user_subscriptions` | Held plans | FK to `users` + `subscription_catalogs`, `valid_from/until`, `subscription_status` (active/inactive/paused/cancelled/expired), `is_primary_mobility_option`, `estimated_usage_frequency`. |
