"""Deterministic mobility analysis engine.

Ingests leg-level travel history and active subscription records, computes factual
usage statistics, detects temporal patterns, and produces a structured summary. No
LLM, no hardcoded thresholds — inefficiencies are flagged only when the math is
unambiguous (e.g. a subscription cost more than it saved).

Pure function (extracted verbatim from the former ``AnalystAgent``). One of the
authoritative sources of every number the frontend reads, so it is called directly
by the /analyze driver and exposed to the Analyst agent only as a tool.
"""
...
def _detect_seasonality(monthly_total: dict) -> str:
    ...
    by_cal_month: dict = defaultdict(list)
...
def analyze_portfolio(travel_history: list, current_subscriptions: list) -> dict:
    ...
    ----------
    travel_history:
        List of trip_leg dicts from the DB (production column names, values already
        coerced by ``clean_row``).
    current_subscriptions:
        List of user_subscriptions rows joined to subscription_catalogs (production
        column names).

    Returns a dict with two logical sections:
    - Full output consumed by the Optimizer and stored in recommendations.
    - ``forecaster_summary`` — a nested dict already shaped to the AnalystSummary
      schema that the forecaster expects.
    """
    ...
    raw_mode_stats: dict = defaultdict(
        lambda: {"trips": 0, "intrinsic": 0.0, "distance": 0.0, "co2": 0.0}
    )
    # disp_mode_stats: grouped display mode (for mode_breakdown output)
    disp_mode_stats: dict = defaultdict(
        lambda: {"trips": 0, "intrinsic": 0.0, "effective": 0.0, "distance": 0.0, "co2": 0.0}
    )
    # sub_stats: per-subscription attribution, keyed by user_subscription_id. Drives
    # subscription_coverage directly from what each leg actually used, rather than
    # inferring coverage from category+mode. This correctly distinguishes flat passes
    # (paid €0) from discount cards like a BahnCard (paid a reduced price) — both
    # report a real "realized_savings" figure.
    sub_stats: dict = defaultdict(
    ...
    for leg in travel_history:
        # reference_cost_eur is the pay-as-you-go price for this leg regardless of any
        # subscription held. Falls back to estimated_cost_eur for legs that predate the
        # field or where no subscription applies.
        intrinsic = float(
    ...
    # ------------------------------------------------------------------ #
    uncovered_spend_by_category: dict = {}
    ...
    # ------------------------------------------------------------------ #
    # 10. Forecaster summary (matches AnalystSummary in forecasting.py)   #
    # ------------------------------------------------------------------ #
    forecaster_summary = {
    ...
    return {
        ...
        # Forecaster-ready summary (used by the forecast engine)
